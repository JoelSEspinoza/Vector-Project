...
Comments:
```{r}
cutadapt <- "C:\\Users\\Joel.Espinoza\\AppData\\Local\\Programs\\Python\\Python37-32\\Scripts\\cutadapt"
system2(cutadapt, args = "--version")
```
^^^This output means it worked
...
```
Comments: I wonder if these new samples used the same primers or not
```{r}
...
```
Comments:These seem to be of very poor quality
...
```
Comments: as to what this step is doing
...
load database
```{r}
Vector.ref <- "C:\\Users\\Joel.Espinoza\\Documents\\ITS2.DB.fasta"
```
...
plot_bar(ps, fill = "Genus") # allows us to visualize by species, so it becomes evident that samples 1, 2, and 7 are likely to be Anopholes Darlingi, but interestingly the majority of the reads from samples3-6 do not appear to match anything. I'm curious as to wear the other species are since we know they exist
plot_bar(ps, fill = "Species")
plot_bar(ps, "Genus", fill = "Species", facet_grid=~Sample) # This allows us to visualize how many of each species the reads match too in comparison to the other species for each sample, which so far tells us the same information, with the exception that we now might be able to discern that the hits for Anopheles gambiae came from sample 5.
